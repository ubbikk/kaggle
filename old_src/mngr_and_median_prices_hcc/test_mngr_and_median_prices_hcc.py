...
def process_other_mngr_medians(train_df, test_df):
    ...
    features = [PRICE, 'num_features', 'num_photos', 'word_num_in_descr',
                ...
                LATITUDE, LONGITUDE, TOTAL_MINUTES]
    new_cols = []
    for f in features:
...
def process_mngr_and_median_ratio_hcc(train_df, test_df):
    ...
    df[mngr_plus_minus] = df['bed_bath_ratio'].apply(lambda s: 'minus' if s<=0 else 'plus')
...
def loss_with_per_tree_stats(df, new_cols):
    ...
    train_df, test_df, new_cols = process_mngr_and_median_ratio_hcc(train_df, test_df)
    train_df, test_df = shuffle_df(train_df), shuffle_df(test_df)
    features += new_cols
...
do_test_with_xgboost_stats_per_tree(1000, 'test_mngr_and_median_prices_hcc', sys.argv[1])