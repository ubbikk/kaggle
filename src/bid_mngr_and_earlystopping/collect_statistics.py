...
def simple_loss(df):
    ...
    test_loss_on_1000 = None
    if len(results_on_test)>1000:
        test_loss_on_1000=results_on_test[1000]
...
def test(num):
    ...
    for i in range(num):
        t=time()
        best_iteration, best_test_loss, current_loss, test_loss_on_1000 = simple_loss(df)
        results.append({
            'best_iteration':best_iteration,
            'best_test_loss':best_test_loss,
            'current_loss':current_loss,
            'test_loss_on_1000':test_loss_on_1000
        })

        print '\n\n'
        print '#{}'.format(i)
        print 'best_iteration       {}'.format(best_iteration)
        print 'avg best_iteration   {}'.format(np.mean([x['best_iteration'] for x in results]))
        print '\n'
        print 'best_test_loss       {}'.format(best_test_loss)
        print 'avg best_test_loss   {}'.format(np.mean([x['best_test_loss'] for x in results]))
        print '\n'
