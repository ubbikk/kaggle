...
def add_log_reg_cols(train_df, test_df, variable, folds, N):
    prior = pd.get_dummies(train_df, columns=[TARGET])[['interest_level_high', 'interest_level_medium', 'interest_level_low']].mean()
    results=[]
    for _ in range(N):
        skf = StratifiedKFold(folds, random_state=int(time()), shuffle=True)
        print int(time())
        ...
        for big_ind, small_ind in skf.split(np.zeros(len(train_df)), train_df[TARGET]):
            big = train_df.iloc[big_ind]
            small = train_df.iloc[small_ind]
            add_log_reg_cols_big_small(big, small, prior, update_df, variable)

    for col in ['log_reg_{}'.format(t) for t in ['high', 'medium', 'low']]:
...
def process_mngr_ens(train_df, test_df):
    ...
    folds = 5
    N=10
    add_log_reg_cols(train_df, test_df, col, folds, N)
...
def loss_with_per_tree_stats(df, new_cols):
    ...
    train_df, test_df, new_cols = process_mngr_ens(train_df, test_df)
    train_df, test_df = shuffle_df(train_df), shuffle_df(test_df)
    features += new_cols
