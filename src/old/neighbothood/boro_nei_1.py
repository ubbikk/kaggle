...
zip_map_file='zip_by_nei_and_boro_corrected.json'

# train_file = '../data/redhoop/train.json'
# test_file = '../data/redhoop/test.json'
# ny_data_file = 'neighbothood/ny.csv'
#
# zip_map_file='neighbothood/zip_by_nei_and_boro_corrected.json'
...
def add_fields_from_nearest_neighbour(df, ny_data):
    ...
    df['boro']=df['zip_code'].apply(lambda s: zip_map[s]['boro'])
    df['nei']=df['zip_code'].apply(lambda s: zip_map[s]['nei'])
...
def loss_with_per_tree_stats(df, new_cols):
    features = ['bathrooms', 'bedrooms', 'latitude', 'longitude', 'price',
                'num_features', 'num_photos', 'word_num_in_descr',
                "created_month", "created_day"]+new_cols
...
def process_boro_nei(df):
    new_cols = get_dummy_cols('nei', set(df['nei']))+get_dummy_cols('boro', set(df['boro']))
    return pd.get_dummies(df, columns=['boro', 'nei']), new_cols
...
def do_test_with_xgboost_stats_per_tree(num, fp):
    ...
    l_1K=[]
    train_df = load_train()
    train_df, new_cols = process_boro_nei(train_df)
    for x in range(num):
        t=time()
        df=train_df.copy()
...
do_test_with_xgboost_stats_per_tree(1000, 'boro_nei_1.json')
#
#
