...
def top_N_ratio(df, N):
    cmap = get_c_map_ordered_features(df[F_COL])
    alll = sum([x[1] for x in cmap])
    topN =sum([x[1] for x in cmap[:N]])
    return float(topN)/alll
