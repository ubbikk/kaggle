...
def loss_with_per_tree_stats(df, new_cols):
    features = [BATH_NORMALIZED, BED_NORMALIZED, 'latitude', 'longitude', 'price',
...
def do_test_with_xgboost_stats_per_tree(num, fp, mongo_host):
    ...
    features = []
    train_df, test_df, new_cols = process_nei12_median(train_df, test_df)
    features += new_cols
...
do_test_with_xgboost_stats_per_tree(1000, 'nei12and_bed_bath_median_prices_test', sys.argv[1])
