...
def do_test(runs):
    df = load_train()
    space = {
        'learning_rate': hp.loguniform('learning_rate', log(0.02), 0),
        'max_depth': hp.choice('max_depth', [2,3,4,5,6]),
        'colsample_bytree':hp.uniform('colsample_bytree',0.6, 1),
        'subsample':hp.uniform('subsample',0.6, 1)
    }
    trials = MongoTrials('mongo://10.20.0.144:27017/xgboost_tuning_bid_mngr/jobs', exp_key='exp1')

    objective = partial(tuning_xgboost_params_mngr_and_bid_optimizer.loss_for_batch, df=df, runs=runs)
