...
def process_nei123(train_df, test_df):
    ...
    beds_vals =[0,1,2,3]
    bath_vals = set(df[BATH_NORMALIZED])
    ...
    for col in [NEI_1, NEI_2, NEI_3]:
        for bed in beds_vals:
            for bath in bath_vals:
                new_col = 'freq_of_{}, with bed={}_and_bath={}'.format(col, bed, bath)
                df[new_col] = df.groupby([col, BED_NORMALIZED, BATH_NORMALIZED])[PRICE].transform('count')
                # df[new_col] = df[new_col]/sz
                new_cols.append(new_col)
